...
// 📌 Root Route (Testing साठी)
app.get('/', (req, res) => {
  res.json({ success: true, message: 'E-Court Board Backend API is running successfully!' });
});
...
app.post('/api/upload-board', upload.fields([
  ...
  try {
    ...
    const civilFile = req.files && req.files['civilPdf'] ? req.files['civilPdf'][0] : null;
    const criminalFile = req.files && req.files['criminalPdf'] ? req.files['criminalPdf'][0] : null;
    ...
    // 💡 FIX 1: Wrap Cloudinary Upload Stream in a Promise for Serverless Execution
    ...
    // Promise पूर्ण होण्याची वाट पाहा (await)
    const result = await uploadToCloudinary(mergedBuffer);
